...
if ( $_SESSION["AUTH"]) {
    ...
    foreach ($resultados as $resultado) {        
        ...
        echo '<a href="index.php?action=photoDetails&id=' . $resultado["idFoto"] . '"><img src="'.$resultado['fichero'].'" alt="' . $resultado['alternativo'] . '"></a>';
        echo '<figcaption>';
    ...
    foreach ($resultados as $resultado) {        
        echo '<figure>';
        echo '<h3>' . $resultado['titulo'] . '</h3>';
        echo  "<img src='".$resultado['fichero']."' alt='" . $resultado['alternativo'] . "'>";
        echo '<figcaption>';
        echo '<p>' . $resultado['descripcion'] . '</p>';
        echo '<p>Country: ' . $resultado['nombre'] . '</p>';
        echo '<p>Fecha: ' . $resultado['fecha'] . '</p>';
